...
const tableHeaders = [
    ...
    { key: 'description', label: 'Equipment Name' },
...
<template>
    ...
    <Table id="site" :headers="tableHeaders" :items="tableData">
        <template #cell-condition="{ value }">
            <span
                :class="['border p-1 px-2 rounded-md text-white', value === 'Working' ? 'bg-green-400' : 'bg-red-400']">
                {{ value }}
            </span>
        </template>
    ...
    <Pagination :links="linksData" :currentPage="currentPage" :lastPage="lastPage" @handleClick="getEquipmentList" />
</template>
